...
class AnalyticsManager {
   ...
   * @param {string} sessionName - Session/player identifier (optional, auto-generated if not provided)
   */
  initialize(gameId, sessionName) {
    ...
    // Format: "session_timestamp_randomstring" - used for both sessionId and name
    this._sessionId = sessionName || `session_${timestamp}_${randomStr}`;
    this._sessionName = this._sessionId;
    
    this._reportData.gameId = gameId;
    this._reportData.sessionId = this._sessionId;
    this._reportData.name = this._sessionName;
  ...
  endLevel(levelId, successful, timeTakenMs, xp) {
    ...
    if (level) {
      ...
      this._reportData.bestXp = Math.max(this._reportData.bestXp, this._reportData.xpEarnedTotal);
  ...
  submitReport() {
    ...
    // Build canonical payload matching PAYLOAD_FORMAT.md specification
    const payload = {
    ...
    console.log('Session Name:', payload.name);
    console.log('Timestamp:', payload.timestamp);
    console.log('Total XP:', payload.xpEarnedTotal);
    console.log('Last Played Level:', payload.lastPlayedLevel);
    console.log('Highest Level:', payload.highestLevelPlayed);
    console.log('Levels Attempted:', payload.diagnostics.levels.length);
    ...
    function trySend(p) {
      ...
      if (!sent) {
        try { console.log('Analytics Payload:', JSON.stringify(p)); } catch (e) { /* swallow */ }
      }
